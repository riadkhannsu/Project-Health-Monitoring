    ...
    <q-page class="column justify-start">
      ...
      <div v-if="showForm" class="col-4 text-center">
        ...
        <q-input
          ...
          type="textarea"
          style="margin:15px"
          hint="Prescription Details"
        />
        <q-btn color="secondary" icon-right="control_point" label="Submit" @click="submitPresc"/>
      </div>
    ...
    submitPresc (evt) {
      ...
      if (this.details === '') {
        ...
        this.$axios.post('http://nickotest.000webhostapp.com/api/get_presc_diagnosis.php', prescData)
          .then(response => {
            this.errMsg = 'Prescription successfully created and submitted'
            this.showForm = false
          })
          .catch(error => {
